<template>
  <main class="mt-5">
    <div class="flex justify-between">
      ...
      <div class="mt-5 pb-4">
        <router-link :to="{ name: 'assignment-create-teacher', params: { courseId: courseId } }">
          <button
            type="button"
    ...
    <div v-for="item in lessonStore.lessonsByCourseId" :key="item.id">
      <div
        class="p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300"
        role="alert"
        @click="loadAssignmentByLesson(item.id)"
      >
        <span class="font-medium font-title-lesson">Lesson: {{ item.name }}</span>
      </div>

      <!-- Kiểm tra xem lesson này có assignments không -->
      <div v-if="assignmentStore.assignmentsByLesson[item.id]">
        <div v-if="assignmentStore.assignmentsByLesson[item.id].length > 0">
          <div
            class="border-assignment"
            v-for="assignment in assignmentStore.assignmentsByLesson[item.id]"
            :key="assignment.id"
          >
            <div class="flex justify-between">
              <div>
                ...
                >
                  <div class="flex justify-between">
                    <p style="font-weight: bold">
                      {{ assignment.name }} - {{ assignment.tag.name }}
                    </p>
                  </div>
                </RouterLink>
                <div class="mt-5 ml-3">
        ...
        <p v-else class="text-gray-500">Không có assignment trong lesson này.</p>
      </div>
    </div>
  </main>
</template>
...
const loadAssignmentByLesson = async (lessonId: number) => {
  await assignmentStore.loadAssignmentByLessonId(lessonId)
  for (const assignment of assignmentStore.assignmentsByLesson[lessonId] || []) {
    assignmentStore.countAssignmentDone[assignment.id] =
...
onMounted(async () => {
  ...
  await courseEnrolledStore.countUserEnrolledCourse(courseId)
})
</script>
<style scoped>
.font-title-course {
