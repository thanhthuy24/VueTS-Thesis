      ...
      <div class="pb-4 ml-7 mt-5 bg-white dark:bg-gray-900">
        ...
        <div class="relative mt-1">
          ...
          <input
            ...
            @input="userStore.search(searchKeyword)"
            type="text"
            id="table-search"
...
const handlePageChange = async (newPage: number) => {
  await userStore.changePage(newPage)
}
...
const closeModal = () => {
  if (selectedUserId.value !== null) {
    const modal = document.getElementById(`popup-modal-${selectedUserId.value}`)
    if (modal) {
      modal.classList.add('hidden') // Nếu dùng Tailwind
    }
    selectedUserId.value = null // Reset lại sau khi đóng modal
  }
}

const updateStatus = async () => {
  if (selectedUserId.value !== null) {
    console.log(`Deleting course ID: ${selectedUserId.value}`)
    await userStore.changeActiveStatus(selectedUserId.value)
    closeModal()
  }
}

onMounted(async () => {
  await userStore.loadUsersByRole()
})
</script>
<style scoped>
.font-large {
